...
function chromium_prebuilt() {
    ...
    appCheck=$T/prebuilts/chromium/$TARGET_DEVICE/app/webview.apk
    device_target=$T/prebuilts/chromium/$TARGET_DEVICE/

    if [ -r $hash ] && [ $(git --git-dir=$T/external/chromium_org/.git --work-tree=$T/external/chromium_org rev-parse --verify HEAD) == $(cat $hash) ] && [ -f $libsCheck ] && [ -f $appCheck ]; then
